...
Use this when Peter asks to write an article from a public `docs.qq.com/doc/...` Tencent Docs link.
...
2. Fetch that URL with the page cookies and a normal browser `User-Agent` / `Referer`.
...
- connect the pattern to Peter's relevant product work when appropriate (for example QCut, article automation, AI-video production graphs).
