...
Use when Peter asks to write an article from a `mp.weixin.qq.com/s/...` source and preserve media.

## Reliable extraction pattern

1. Run the Baoyu URL-to-Markdown workflow first to capture text/title/author/date:
   ```bash
   npx -y bun /Users/peter/.hermes/skills/openclaw-imports/qcut-toolkit/baoyu/baoyu-url-to-markdown/scripts/main.ts '<wechat-url>' -o /tmp/wechat-source.md --timeout 60000
   ```
