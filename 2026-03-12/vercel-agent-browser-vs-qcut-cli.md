...
## 更新：agent-browser v0.19 发布

本文发布两天后，agent-browser 推出了 v0.19——**这是全面转向 Rust 之前的最后一个版本**。

### v0.19 新增了什么

- **@browserless provider**：新增 Browserless 作为云端浏览器提供商，扩展了 browserbase / browseruse / kernel 之外的部署选项
- **剪贴板命令**：`clipboard read` / `clipboard write` / `clipboard copy` / `clipboard paste`——Agent 现在可以通过剪贴板与页面交互，这在处理富文本编辑器等场景时很实用
- **截图控制增强**：新增 `--screenshot-quality`、`--screenshot-format`、`--screenshot-dir` 参数，精确控制截图输出
- **Bug 修复**：多项稳定性改进
...
作者 [@ctatedev](https://x.com/ctatedev/status/2032383712029982883) 明确表示 v0.19 是"the final release before going 100% Rust"。

这意味着：
- 当前的 Node + Playwright 路径将被 Rust native 完全接管
- 启动速度、内存占用、分发体积都会大幅改善
- 对于做 sidecar 集成的团队（包括 QCut），**现在是锁定 v0.19 接口做适配的最佳窗口**——等 Rust 版稳定后再升级

### 对 Builder 的实际影响

1. **现在集成不亏**：v0.19 的命令接口已经很稳定，Rust 版承诺保持兼容
2. **剪贴板命令填补了实际空白**：之前要操作剪贴板得绕很多弯路
3. **多 provider 选择降低了锁定风险**：Browserless 加入意味着你不必绑定单一云端方案
4. **关注 Rust 迁移节奏**：如果你依赖 Node 侧的内部 API，现在就要开始做隔离层
